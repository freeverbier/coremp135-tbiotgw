...
setup_tb_gateway() {
    ...
    # Device name: colons → dashes (valid hostname chars, clean TB device name)
    # e.g. 8a:65:34:49:34:30  →  coremp135-8a-65-34-49-34-30
    local device_name="coremp135-${mac_address//:/-}"
    ...
    # Generate docker-compose.yml (single-quoted heredoc — no bash expansion inside)
...
services:
  tb-gateway:
    ...
    restart: always
    # hostname injected by setup.sh below
...
COMPOSEFILE
    ...
    # Inject hostname into docker-compose.yml (after the single-quoted heredoc)
    # This sets the container hostname so TB Gateway uses it as the device name.
    sed -i "s/    # hostname injected by setup.sh below/    hostname: ${device_name}/" \
        "${INSTALL_DIR}/docker-compose.yml"
    log "docker-compose.yml: hostname set to ${device_name}"

    # Generate runtime .env (never committed to git)
    # Set ALL known TB Gateway env var names for device name (compatibility across versions)
...
# Device name — set via all known variable names for cross-version compatibility
TB_GW_DEVICE_NAME=${device_name}
TB_GW_PROVISIONING_DEVICE_NAME=${device_name}
ENVFILE
...
# -----------------------------------------------------------------------------
# Read a token from tb_gateway.json inside the container.
# Uses python3 if available, falls back to grep+sed (shell-only).
...
patch_tb_gateway_config() {
    ...
    local max_attempts=36   # 3 min max (36 × 5s)
    local attempt=0
    local token=""

    while [[ $attempt -lt $max_attempts ]]; do
        token=$(_read_tb_token)
        ...
        # Progress log every 30s so the user knows it's still running
        if (( attempt % 6 == 0 )); then
            log "  still waiting for provisioning... ($((attempt * 5))s elapsed)"
        fi
        sleep 5
        attempt=$((attempt + 1))
    done
    ...
    if [[ $attempt -ge $max_attempts ]]; then
        warn "Timed out waiting for provisioning (3 min) — skipping remote config patch"
        warn "The gateway will still work. Run the following manually once provisioned:"
        warn "  docker exec tb-gateway sh -c 'python3 -c \"...\"'"
        return 0
    fi

    # ---- Step 3: Patch tb_gateway.json with remoteLogging + remoteShell ----------
    log "Patching gateway config: remoteLoggingLevel=DEBUG, remoteShell=true..."
...
with open(cfg, "w") as f:
    json.dump(d, f, indent=2)
print("JSON patched OK")
PYEOF
else
    # Fallback: inject via sed if remoteLoggingLevel not present
    if ! grep -q "remoteLoggingLevel" "$CFG" 2>/dev/null; then
        sed -i "s/\"thingsboard\"[[:space:]]*:{/\"thingsboard\":{\"remoteLoggingLevel\":\"DEBUG\",\"remoteShell\":true,/" "$CFG" || true
        echo "JSON patched via sed"
    fi
fi
' 2>&1 | while IFS= read -r line; do log "  [json-patch] $line"; done \
    || { warn "Config patch failed — gateway will still work, remote logging disabled"; return 0; }

    # Restart to apply new config
    log "Restarting TB Gateway to apply patched config..."
...
main() {
    ...
    local mac_address device_name
    mac_address=$(get_eth0_mac)
    device_name="coremp135-${mac_address//:/-}"
    ...
    patch_tb_gateway_config "$device_name"   # patch YAML name + wait for provisioning + enable remote logging
    setup_autostart
    install_tailscale
    setup_attribute_reporter
    setup_display
